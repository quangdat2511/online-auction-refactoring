...
export function findLevel2Categories() {
  return db('categories').whereNot('parent_id', null);
}
...
export function updateCategory(id, category) {
  return db('categories').where('id', id).update(category).returning('*');
}
...
export function findChildCategoryIds(parentCategoryId) {
  return db('categories').where('parent_id', parentCategoryId).select('id');
}
